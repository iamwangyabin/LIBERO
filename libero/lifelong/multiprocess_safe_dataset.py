...
class MultiprocessSafeGroupedTaskDataset(Dataset):
    ...
    # 透传分组数据集的关键统计属性，供上层直接访问
    def __getattr__(self, name):
        if name.startswith("_"):
            raise AttributeError(f"{self.__class__.__name__} has no attribute '{name}'")
        if name in {"task_group_size", "n_demos", "total_num_sequences"}:
            # 这些在 __init__ 期间已缓存，可直接返回
            if name in self.__dict__:
                return self.__dict__[name]
        # 其他属性尝试从底层分组数据集透传
        self._ensure_datasets_initialized()
        if self._grouped_dataset is not None and hasattr(self._grouped_dataset, name):
            return getattr(self._grouped_dataset, name)
        raise AttributeError(f"{self.__class__.__name__} has no attribute '{name}'")

    def __dir__(self):
        base = set(super().__dir__())
        if self._grouped_dataset is not None:
            base.update(dir(self._grouped_dataset))
        base.update({"task_group_size", "n_demos", "total_num_sequences"})
        return sorted(base)
